...
static void print_usage(const char* argv0) {
    std::cerr
        << "Usage:\n"
        << "  " << argv0
        << " [--config <server_pubkeys.json>] [--json] generate-request <product_id> "
           "[customer_hint]\n"
...
static int cmd_generate_request(const std::vector<std::string>& args,
                                const license::client::ServerKeys& keys, bool as_json) {
...
static int cmd_verify(const std::vector<std::string>& args, const license::client::ServerKeys& keys,
                      bool as_json) {
...
static int cmd_deactivate(const std::vector<std::string>& args, bool as_json) {
    ...
    if (as_json) {
        std::cout << json{{"success", ok}, {"product_id", product_id}}.dump(2) << "\n";
    } else {
        if (ok)
            std::cout << "[OK] License deactivated for: " << product_id << "\n";
        else
            std::cerr << "Error: deactivation failed.\n";
    }
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "fingerprint")
        return cmd_fingerprint(as_json);
    if (cmd == "deactivate")
        return cmd_deactivate(args, as_json);
    ...
    if (cmd == "generate-request")
        return cmd_generate_request(args, keys, as_json);
    if (cmd == "activate")
        return cmd_activate(args, keys, as_json);
    if (cmd == "verify")
        return cmd_verify(args, keys, as_json);
