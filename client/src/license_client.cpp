...
ServerKeys load_server_keys(const std::string& config_path) {
    ...
    if (!f)
        throw std::runtime_error(
            "Cannot open server key config: " + config_path +
            "\nRun: license-server init-keys --keystore <path> --pub-out " + config_path);
                                       ...
                                       const ServerKeys&  keys)
{
    ...
    try {
        ...
        req.timestamp             = static_cast<int64_t>(
...
ActivateResult activate(const std::string& token_b64,
                        const ServerKeys&  keys)
{
...
VerifyResult verify_at_startup(const std::string& product_id,
                               const ServerKeys&  keys)
{
    VerifyResult result;
    try {
