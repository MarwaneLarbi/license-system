...
static UniqueTbs open_tbs() {
    ...
    TBS_RESULT rc = Tbsi_Context_Create(reinterpret_cast<PCTBS_CONTEXT_PARAMS>(&params), h.get());
    if (rc != TBS_SUCCESS) {
        char msg[64];
...
static Bytes submit(TBS_HCONTEXT hctx, const Bytes& cmd) {
    ...
    TBS_RESULT rc =
        Tbsip_Submit_Command(hctx, TBS_COMMAND_LOCALITY_ZERO, TBS_COMMAND_PRIORITY_NORMAL,
                             cmd.data(), static_cast<UINT32>(cmd.size()), resp.data(), &resp_len);
    if (rc != TBS_SUCCESS) {
        char msg[64];
    ...
    if (resp_len < 10)
        throw std::runtime_error("TPM response too short");
    uint32_t rc_val = (static_cast<uint32_t>(resp[6]) << 24) |
                      (static_cast<uint32_t>(resp[7]) << 16) |
                      (static_cast<uint32_t>(resp[8]) << 8) | static_cast<uint32_t>(resp[9]);
    if (rc_val != 0) {
        char msg[64];
...
Bytes WindowsTpmProvider::Impl::sign_with_device_key(const std::string& h,
