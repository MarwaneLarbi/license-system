...
std::string cpu_id() {
    ...
    // CPU brand string via CPUID leaves 0x80000002..4.
...
#  endif
    ...
    // Also append family/model/stepping hex.
#  ifdef _WIN32
    __cpuid(regs.data(), 1);
    uint32_t sig = static_cast<uint32_t>(regs[0]);
#  else
    if (__get_cpuid(1, &a, &b, &c, &d)) {
        uint32_t sig = a;
        char hex[16];
        snprintf(hex, sizeof(hex), "-%08X", sig);
        result += hex;
    }
#  endif
    return result;
...
FingerprintResult compute(bool use_tpm) {
    ...
    if (!ek_str.empty())   result.components.push_back("tpm_ek:"    + ek_str.substr(0, 16) + "...");
    if (!uuid.empty())     result.components.push_back("bios_uuid:" + uuid);
    if (!cpu.empty())      result.components.push_back("cpu:"       + cpu);
    if (!disk.empty())     result.components.push_back("disk:"      + disk);
