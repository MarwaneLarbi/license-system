...
std::optional<crypto::Bytes> tpm_linux_ek_pub_hash() {
    try {
        ...
        TPML_PCR_SELECTION     pcr_sel{ .count = 0 };
        ...
        TSS2_RC rc = Esys_CreatePrimary(
            ...
            &sensitive, &ek_template, &outside_info, &pcr_sel,
        ...
        std::size_t offset = 0;
        rc = Tss2_MU_TPM2B_PUBLIC_Marshal(ek_pub, pub_bytes.data(),
                                           pub_bytes.size(), &offset);
        ...
        return crypto::sha256(
            std::span<const uint8_t>{ pub_bytes.data(), offset });
    } catch (...) {
        return std::nullopt;
    }
}
...
static ESYS_TR create_pcr_policy_session(ESYS_CONTEXT* esys) {
    ...
    TSS2_RC rc = Esys_StartAuthSession(
        ...
        &(TPMT_SYM_DEF){ .algorithm = TPM2_ALG_AES,
                          .keyBits.aes = 128,
    ...
    pcr_sel.pcrSelections[0].pcrSelect[0] =
        (1u << 0) | (1u << 7); // PCR 0 and PCR 7

    rc = Esys_PolicyPCR(esys, session,
                        ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                        nullptr, &pcr_sel);
...
public:
    LinuxTpmProvider() { esys_ = open_esys(); }
...
private:
    ...
    ESYS_TR _get_storage_parent() {
        ...
        TSS2_RC rc = Esys_CreatePrimary(
            esys_.get(), ESYS_TR_RH_OWNER,
            ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE,
            &sens, &primary_template, &outside, &pcr,
            &parent, nullptr, nullptr, nullptr, nullptr);
    ...
    Bytes _seal_to_tpm(const Bytes& secret, const std::string& /*label*/) {
        ...
        TSS2_RC rc = Esys_PolicyGetDigest(esys_.get(), policy_session,
            ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE, &policy_digest);
        ...
        TPM2B_SENSITIVE_CREATE sens{};
        sens.size = 0;
        sens.sensitive.data.size = static_cast<uint16_t>(
            std::min(secret.size(), static_cast<std::size_t>(128)));
        std::memcpy(sens.sensitive.data.buffer,
                    secret.data(), sens.sensitive.data.size);
        ...
        std::memcpy(sp.authPolicy.buffer, policy_digest->buffer,
                    policy_digest->size);
        ...
        rc = Esys_Create(esys_.get(), parent,
            ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE,
            &sens, &seal_template, &outside, &pcr_sel,
            &priv_out, &pub_out, nullptr, nullptr, nullptr);
        ...
        free(priv_out); free(pub_out);

        // Blob = 4-byte priv_len || priv || 4-byte pub_len || pub.
        Bytes blob;
        auto push32 = [&](uint32_t v) {
            blob.push_back(v >> 24); blob.push_back((v >> 16) & 0xff);
    ...
    Bytes _unseal_from_tpm(const Bytes& blob, const std::string& /*label*/) {
        ...
        TSS2_RC rc = Esys_Load(esys_.get(), parent,
            ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE,
        ...
        rc = Esys_Unseal(esys_.get(), obj_handle,
        ...
        Bytes result(sensitive_out->buffer,
                     sensitive_out->buffer + sensitive_out->size);
        free(sensitive_out);
        return result;
    }
};
