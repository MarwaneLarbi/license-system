...
UniqueEvpPkey evp_pkey_from_raw_priv(int nid, const uint8_t* raw, std::size_t len) {
    UniqueEvpPkey key{EVP_PKEY_new_raw_private_key(nid, nullptr, raw, len)};
    if (!key)
        throw_openssl("EVP_PKEY_new_raw_private_key");
    return key;
}

/// Load a raw public key for an EVP algorithm.
UniqueEvpPkey evp_pkey_from_raw_pub(int nid, const uint8_t* raw, std::size_t len) {
...
std::string base64_encode(std::span<const uint8_t> data) {
    ...
    while (rem >= 3) {
        uint32_t v = (static_cast<uint32_t>(p[0]) << 16) | (static_cast<uint32_t>(p[1]) << 8) |
...
Bytes base64_decode(std::string_view b64) {
    ...
    static const auto DEC = []() {
        ...
        t.fill(-1);
        for (int i = 0; i < 64; ++i)
            t[static_cast<uint8_t>(B64_CHARS[i])] = static_cast<int8_t>(i);
        t['='] = 0;
        return t;
    }();
...
Bytes hkdf_sha256(std::span<const uint8_t> ikm, std::span<const uint8_t> salt,
                  std::span<const uint8_t> info, std::size_t len) {
    UniqueEvpKdf kdf{EVP_KDF_fetch(nullptr, "HKDF", nullptr)};
    if (!kdf)
        throw_openssl("EVP_KDF_fetch HKDF");
    ...
    params[pi++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY,
...
Bytes aes256gcm_encrypt(std::span<const uint8_t> key, std::span<const uint8_t> aad,
    ...
    if (!aad.empty()) {
        int outlen = 0;
        if (EVP_EncryptUpdate(ctx.get(), nullptr, &outlen, aad.data(),
    ...
    if (plain.empty()) {
        ...
        if (EVP_EncryptUpdate(ctx.get(), ct.data(), &len1, plain.data(),
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_GET_TAG, static_cast<int>(AeadParams::TAG_LEN),
                            tag.data()) != 1)
...
Bytes aes256gcm_decrypt(std::span<const uint8_t> key, std::span<const uint8_t> aad,
    ...
    auto ct = ciphertext.subspan(AeadParams::NONCE_LEN, ciphertext.size() - OVERHEAD);
    // Tag is at the end (mutable copy for OpenSSL).
    Bytes tag(ciphertext.end() - AeadParams::TAG_LEN, ciphertext.end());

    UniqueCipherCtx ctx{EVP_CIPHER_CTX_new()};
    if (!ctx)
        throw_openssl("EVP_CIPHER_CTX_new (dec)");
    ...
    if (!aad.empty()) {
        int outlen = 0;
        if (EVP_DecryptUpdate(ctx.get(), nullptr, &outlen, aad.data(),
                              static_cast<int>(aad.size())) != 1)
    ...
    int len1 = 0;
    if (!ct.empty()) {
        if (EVP_DecryptUpdate(ctx.get(), plain.data(), &len1, ct.data(),
                              static_cast<int>(ct.size())) != 1)
            throw_openssl("EVP_DecryptUpdate (dec)");
    }

    // Set expected tag.
    if (EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_SET_TAG, static_cast<int>(AeadParams::TAG_LEN),
                            tag.data()) != 1)
...
Bytes x25519_exchange(std::span<const uint8_t> our_priv, std::span<const uint8_t> peer_pub) {
...
Bytes ed25519_sign(std::span<const uint8_t> priv_key, std::span<const uint8_t> message) {
    // Accept either 32-byte seed or 64-byte seed||pub.
    auto key = evp_pkey_from_raw_priv(EVP_PKEY_ED25519, priv_key.data(), 32);

    UniqueEvpMdCtx ctx{EVP_MD_CTX_new()};
    if (!ctx)
        throw_openssl("EVP_MD_CTX_new Ed25519 sign");
...
bool ed25519_verify(std::span<const uint8_t> pub_key, std::span<const uint8_t> message,
                    std::span<const uint8_t> signature) {
    auto key = evp_pkey_from_raw_pub(EVP_PKEY_ED25519, pub_key.data(), pub_key.size());

    UniqueEvpMdCtx ctx{EVP_MD_CTX_new()};
    if (!ctx)
        throw_openssl("EVP_MD_CTX_new Ed25519 verify");
    ...
    int rc = EVP_DigestVerify(ctx.get(), signature.data(), signature.size(), message.data(),
                              message.size());
    if (rc < 0)
        throw_openssl("EVP_DigestVerify error");
...
Bytes hybrid_encrypt(std::span<const uint8_t> recipient_pub, std::span<const uint8_t> aad,
    ...
    auto aes_key = hkdf_sha256(
        dh_secret, ephemeral.pub,
        std::span<const uint8_t>{reinterpret_cast<const uint8_t*>(HYBRID_HKDF_INFO.data()),
...
Bytes hybrid_decrypt(std::span<const uint8_t> recipient_priv, std::span<const uint8_t> aad,
                     std::span<const uint8_t> ciphertext) {
    constexpr std::size_t EPH_PUB_LEN = 32;
    constexpr std::size_t MIN_LEN = EPH_PUB_LEN + AeadParams::NONCE_LEN + AeadParams::TAG_LEN;
